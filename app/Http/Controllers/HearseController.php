...
class HearseController extends Controller
{
    ...
    public function hearseallclisting(Request $request)
    {
        ...
        $selectQryStr = "";
        if( $allUsersData ){
            $selectQryStr = "<select class=\"form-control\" name=\"users_ids\" data-toggle=\"select2\" multiple=\"\">";
                    ...
                    switch ($caseId) {
                        case 1:
                            if( !empty($hearsesOrdersVal->booked_to_day) ) {
                                $first_from_date = date("d-m-Y", strtotime($hearsesOrdersVal->booked_from_day));
                                $first_date = date("d-m-Y", strtotime($hearsesOrdersVal->booked_from_day));
                        ...
                        case 2:
                            if( !empty($hearsesOrdersVal->booked_to_day) ) {
                                $second_from_date = date("d-m-Y", strtotime($hearsesOrdersVal->booked_from_day));
                                $second_date = date("d-m-Y", strtotime($hearsesOrdersVal->booked_from_day));
                        ...
                        case 3:
                            if( !empty($hearsesOrdersVal->booked_to_day) ) {
                                $third_from_date = date("d-m-Y", strtotime($hearsesOrdersVal->booked_from_day));
                                $third_date = date("d-m-Y", strtotime($hearsesOrdersVal->booked_from_day));
    ...
    */
    ...
    public function saveForm(Request $request){
        ...
        $order->booked_to_day = date("Y-m-d", strtotime(str_replace("/","-",$request->get("booked_from_day"))));
