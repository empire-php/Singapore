...
class SettingsController extends Controller
{
	...
	public function editRosterSettings( Request $request ){
        ...
        $data["id"] = $request->get("id","");
        if ($request->get("id")){
            $data["roster"] = Packages::find($request->get("id"));
        }
        else{
            $data["roster"] = new Packages();
        }
	 ...
	 public function saveRosterSettings( Request $request ){
        $data["id"] = $request->get("id","");
             if ($request->get("team_name")){
                ...
                $createNew = true;
                if ($request->get("id")){
                    $oldRosterSetting = RosterSettings::find($request->get("id"));


                    if ($request->get("team_name") == $oldRosterSetting->team_name){
                        $createNew = false;
                    }

                    if ($createNew){                               
                        $rostersetting->team_name = $oldRosterSetting->team_name;
                        $oldPackage->is_deleted = 1;
                        $oldPackage->save();
                    }
                }
                ...
                if ($createNew){
                    ...
                    $rostersetting->team_leader = $request->get("team_leader");
                    $rostersetting->embalmers = $request->get("embalmers");
                    $rostersetting->others = $request->get("others");
                ...
                if (!$createNew){
                    $rostersetting = $oldRosterSetting;
                }
            }
            else{
                $rostersetting = new Packages();
            }
        
        $data["roster"] = $rostersetting;
        return view('settings.edit_roster_settings', $data);
    }
