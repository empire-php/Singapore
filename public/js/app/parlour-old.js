$(document).ready(function(){
   ...
   if ($(".parlour_form").find("#order_id").val()){
    ...
    $(".parlour_form").find("#booked_from_day").on("change", function (e) {
        $(".parlour_form").find("#booked_to_day").val("");
        myDatePicker.datepicker('setStartDate', form.find("#booked_from_day").val());
    ...
    $("#submit_bttn_form").click(function(e){
        e.preventDefault();
        
        if ($(".selected_item .parlour_capacity_container").text() && parseInt($("#capacity").val()) > parseInt($(".selected_item .parlour_capacity_container").text())){
            alert("Capacity is too high");
        }
        else{
            $(".master_form").submit();
        }
